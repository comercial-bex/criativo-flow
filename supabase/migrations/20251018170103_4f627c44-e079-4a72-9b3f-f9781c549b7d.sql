...
CREATE OR REPLACE FUNCTION public.trg_update_pessoas_updated_at()
...
-- Criar função de normalização primeiro
CREATE OR REPLACE FUNCTION public.normalizar_cpf(cpf_input TEXT) RETURNS TEXT LANGUAGE plpgsql IMMUTABLE AS $$ 
BEGIN 
  IF cpf_input IS NULL THEN RETURN NULL; END IF; 
  RETURN regexp_replace(cpf_input, '\D', '', 'g'); 
END; $$;

-- Migrar profiles
INSERT INTO public.pessoas (id, profile_id, nome, email, telefones, papeis, status, cliente_id, observacoes, created_at, updated_at)
SELECT gen_random_uuid(), pr.id, pr.nome, pr.email, 
  CASE WHEN pr.telefone IS NOT NULL AND pr.telefone != '' THEN ARRAY[pr.telefone] ELSE '{}'::text[] END, 
  CASE WHEN pr.cliente_id IS NOT NULL THEN ARRAY['cliente'] ELSE '{}'::text[] END, 
  pr.status, pr.cliente_id, pr.observacoes_aprovacao, pr.created_at, pr.updated_at 
FROM public.profiles pr;

-- Migrar rh_colaboradores (NORMALIZANDO CPF)
...
SELECT 
  c.id, 
  c.nome_completo, 
  normalizar_cpf(c.cpf_cnpj), 
  c.email, 
  CASE WHEN c.celular IS NOT NULL AND c.celular != '' THEN ARRAY[c.celular] ELSE '{}'::text[] END, 
  ARRAY['colaborador'], 
  c.cargo_atual, 
  c.regime, 
  c.status, 
...
ON CONFLICT (id) DO UPDATE SET 
  papeis = CASE WHEN 'colaborador' = ANY(pessoas.papeis) THEN pessoas.papeis ELSE array_append(pessoas.papeis, 'colaborador') END;

-- Popular pessoa_papeis
INSERT INTO public.pessoa_papeis (pessoa_id, papel, ativo, data_inicio)
SELECT p.id, unnest(p.papeis), TRUE, COALESCE(p.data_admissao, p.created_at::date) 
FROM public.pessoas p WHERE array_length(p.papeis, 1) > 0;
...
-- Funções auxiliares
CREATE OR REPLACE FUNCTION public.adicionar_papel_pessoa(p_pessoa_id UUID, p_papel TEXT, p_data_inicio DATE DEFAULT CURRENT_DATE) RETURNS UUID LANGUAGE plpgsql SECURITY DEFINER SET search_path = public AS $$ 
DECLARE v_papel_id UUID; 
BEGIN 
  IF p_papel NOT IN ('colaborador', 'especialista', 'fornecedor', 'cliente') THEN RAISE EXCEPTION 'Papel inválido: %', p_papel; END IF; 
  INSERT INTO public.pessoa_papeis (pessoa_id, papel, ativo, data_inicio) VALUES (p_pessoa_id, p_papel, TRUE, p_data_inicio) ON CONFLICT (pessoa_id, papel, data_inicio) DO UPDATE SET ativo = TRUE RETURNING id INTO v_papel_id; 
  UPDATE public.pessoas SET papeis = CASE WHEN p_papel = ANY(papeis) THEN papeis ELSE array_append(papeis, p_papel) END, updated_at = NOW() WHERE id = p_pessoa_id; 
  RETURN v_papel_id; 
END; $$;

CREATE OR REPLACE FUNCTION public.remover_papel_pessoa(p_pessoa_id UUID, p_papel TEXT) RETURNS BOOLEAN LANGUAGE plpgsql SECURITY DEFINER SET search_path = public AS $$ 
BEGIN 
  UPDATE public.pessoa_papeis SET ativo = FALSE, data_fim = CURRENT_DATE WHERE pessoa_id = p_pessoa_id AND papel = p_papel AND ativo = TRUE; 
  UPDATE public.pessoas SET papeis = array_remove(papeis, p_papel), updated_at = NOW() WHERE id = p_pessoa_id; 
  RETURN TRUE; 
END; $$;