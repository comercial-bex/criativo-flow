-- Corrigir função get_user_complete - remover referência a coluna inexistente p.nome
-- A tabela auth.users não tem coluna 'nome', apenas a tabela pessoas tem
...
COMMENT ON FUNCTION get_user_complete(UUID) IS 'Retorna perfil completo do usuário (profile + pessoa + role) em 1 query - CORRIGIDO: removido p.nome que não existe em auth.users';